...
server <- function(input, output, session){
  ...
  banco_geral <- read_excel("banco_geral.xlsx")
  
  alocados<-banco_geral%>%
    select(2,3,4,7,13,14:34)
  
  # banco de dados materias
  materias_alocados<- alocados%>%
    select(7:22, 24)
  ...
  # Extraindo somente o nome da matéria
  materias_alocados$Matéria <- str_remove(materias_alocados$Matéria, ".*de ")  # Remove tudo até "de "
  ...
  banco_geral <- read_excel("banco_geral.xlsx") # banco com os alocados
  
  alocados<-banco_geral%>%
    select(2,3,4,7,13,14:34)
  
  output$frequencia_projeto_x_alocado <- renderPlotly({
    df_freq_alocados <- alocados %>%
      mutate(`Membros alocados` = str_split(`Membros alocados`, ",\\s*")) %>%
      unnest(`Membros alocados`) %>%
      group_by(`Membros alocados`) %>%
      summarise(n = n()) %>%
    ...
    fig <- plot_ly(
      df_freq_alocados,
      y = ~n,
      x = ~reorder(`Membros alocados`, n),
      color = ~`Membros alocados`,
      text = ~freq,
      type = "bar",
