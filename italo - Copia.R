...
banco_recem_atualizado <- read_excel("controle_de_projetos_21-03-2025.xlsx")
...
names(df) <- c("nome_projeto", "fase_projeto", "etiquetas", "data_vencimento", 
               "gerente", "assessor_vendas", "gestor", "POWER BI", "Exploratória", 
               "Delineamento", "Métodos 2", "Amostragem", "Regressão Linear", 
               "Multivariada", "inicio_execucao", "fim_execucao", "tempo_total_execucao")
...
server <- function(input, output, session){
  ...
  df_projetos <- reactive({
    if (is.null(input$upload)) {
      # Sem upload, usa o banco padrão atualizado
      df <- banco_recem_atualizado
    } else {
      # Caso o usuário faça upload de um arquivo, lê o arquivo
      file <- input$upload
      ...
      if (ext == "csv") {
        ...
        showNotification("Tipo de arquivo não suportado", type = "error")
        return(NULL)
      }
    }
    
    # Atualiza os nomes das colunas conforme o novo banco
    names(df) <- c("nome_projeto", "fase_projeto", "etiquetas", "data_vencimento", 
                   "ods", "gerente", "comercial", "gestor", "valor_projeto", "materias", 
                   "inicio_execucao", "fim_execucao", "tempo_total_execucao")
    
    # Realiza as transformações necessárias (ex: extração de pf/pj, cálculo de atraso, etc.)
    df <- df %>%
      mutate(pf_pj = str_extract(nome_projeto, "\\w{2}"), 
             atraso = bizdays(data_vencimento, fim_execucao)) %>%  # atraso positivo = atraso; negativo = adiantado
