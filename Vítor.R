...
extrair_top_materias <- function(dataframe, gerente_da_hora){
  ordem <- dataframe%>%
    ...
    unnest(gerente) %>%
    mutate(assuntos = str_split(etiquetas, ",\\s*")) %>%
    unnest(assuntos) %>%
...
banco_recem_atualizado <- read_excel("C:/Users/Vítor/Documents/ESTAT/Projetos 2025.1/OKR's/Otimização Dashboard/controle_de_projetos_21-03-2025.xlsx")
  ...
  dashboardPage(
    ...
    dashboardSidebar(sidebarMenu(
      ...
      menuItem("Estudo", icon = icon("chart-line"),
               menuSubItem("Alocados", tabName = "estudo-alocados"),
    ...
    dashboardBody(
      ...
      tabItems(
        ...
        tabItem(tabName = "estudo-alocados",
                box(plotlyOutput("frequencia_projeto_x_alocado"),width = 12, status = "success", title = "Frequência de Projetos"),
                box(DTOutput("materias_alocado"), width = 12, status = "danger", title = "Matérias por alocado")
        ),
...
server <- function(input, output, session){
  ...
  df_projetos <- reactive({
    ...
    df<- banco_recem_atualizado%>%
      select(c(2:5,17:19,25:31,39:41))
    
    names(df) <- c("nome_projeto", "fase_projeto", "etiquetas", "data_vencimento", "gerente", "assessor_vendas", "gestor", "POWER BI", "Exploratória", "Delineamento", "Métodos 2", "Amostragem", "Regressão Linear", "Multivariada", "inicio_execucao", "fim_execucao", "tempo_total_execucao")
    
    
    
    df <- df %>%
      mutate(atraso = bizdays(data_vencimento, fim_execucao)) %>%#atraso é positivo, numeros negativos sao datas adiantadas
  ...
  output$nps <- renderValueBox({
    ...
    no_aguardo_projetos <- df_projetos() %>%
      filter(fase_projeto %in% c("CSAT Final", "Feedback Interno")) 
  ...
  output$tbl_geral <- renderDT({
    ...
    banco <- df_projetos() %>%
      select(c(nome_projeto, inicio_execucao, data_vencimento, fim_execucao, gerente, etiquetas, fase_projeto, atraso)) %>%
      mutate(
    ...
    names(banco) <- c("Projeto", "Data de Início", "Data de Fim(Esperado)", "Data de Fim(Observado)", "Gerentes", "Matérias", "Fase", "Atraso(dias úteis)*")
  ...
  output$tipos_de_projetos <- renderPlotly({
    ...
    df_projetos() %>%
      mutate(topicos = str_split(etiquetas, ",\\s*")) %>%
      unnest(topicos) %>%
      group_by(topicos) %>%
      summarise(n = n()) %>%
      arrange(desc(n)) %>%
  ...
  createPlot2 <- reactive({
    ...
    tasks <- df_projetos() %>% 
      filter(fase_projeto %in% c("Caixa de entrada", "Execução", "Pausado")) %>%
  ...
  ## Tabela matérias por alocado 
  
  ## Principais matérias por alocado
  materias_alocados<- df%>%
    select(8:14)
  
  materias_alocados<-as.data.frame(t(materias_alocados)) # transpondo linhas para colunas
  
  # colocar as matérias como uma coluna
  materias_alocados <- materias_alocados %>%
    rownames_to_column(var = "Matéria")
  
  # empilhando as colunas com os membros
  materias_alocados <- materias_alocados %>%
    pivot_longer(cols = -Matéria, 
                 names_to = "Tipo_Conteudo", 
                 values_to = "Membro")
  
  materias_alocados<-materias_alocados%>%
    select(1,3) # retirando a colunas com V1, V2, ... gerada pelo R
  
  # Separando os nomes dentro das células e empilhando
  materias_alocados <- materias_alocados %>%
    separate_rows(Membro, sep = ", ")%>%
    na.omit(materias_alocados)
  
  # agrupando por pessoa e matéria
  df_frequencia_materias<-materias_alocados%>%
    group_by(Membro, Matéria) %>%
    summarise(n = n())%>%
    rename(Frequência = n)
  df_frequencia_materias<-df_frequencia_materias%>%
    arrange(Membro,desc(Frequência)) # ordem do maior numero de materias por membro
  
  output$materias_alocado <- renderDT({
    datatable(df_frequencia_materias,options = list(pageLength = length(df_frequencia_materias), 
                                                    language = list(url = "https://cdn.datatables.net/plug-ins/2.1.3/i18n/pt-BR.json"), na = "--"), filter = "top")
  })
  
  
  ## Gráfico de frequência de projetos por alocado
  
  output$frequencia_projeto_x_alocado <- renderPlotly({
    df_freq_alocados <- materias_alocados %>%
      group_by(Membro) %>%
      summarise(n = n()) %>%
      arrange(desc(n))
  ...
  ## Gráfico de frequência de projetos por gerente
  ...
  ## Gráfico de linhas de projetos gerenciados por gerente ao longo do tempo
  output$linha_temporal_projetos_gerenciados <- renderPlotly({
    ...
    projetos_por_gerente<-projetos_expandidos%>%
      mutate(periodo = as.Date(periodo)) # transformar para somente pegar a data (sem o horário)
    
    projetos_por_gerente <- projetos_por_gerente %>%
      group_by(periodo, gerente) %>%
    ...
    ggplot(projetos_por_gerente, aes(x = periodo, y = qtd_projetos, color = gerente, group = gerente)) +
      ...
      scale_y_continuous(breaks = seq(min(projetos_por_gerente$qtd_projetos), max(projetos_por_gerente$qtd_projetos), by = 1))+
      theme_minimal()
  ...
  ## Tabela de principais matérias por gerente de projetos 
  ...
  ## Card do projeto no pipefy
  output$card_pipefy <- renderUI({
    card_pipefy <- df_projetos() %>%
      filter(nome_projeto == input$filter_pipefy) %>%
      setNames(c("Nome", "Fase", "Etiquetas", "Vencimento", 
                 "Gerente de Projetos", "Assessor de Vendas", "Assessor de Gestão", "Execução(início)", "Execução(fim)", 
                 "Duração da Execução", "Atraso")) %>%
      t() %>%
      as.data.frame()
  ...
  ## Gráfico de números de esperado e observado
  ...
  ## Gráfico de números de dias de atraso (ou antecedência) de projetos finalizados
